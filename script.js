...
function start() {
    ...
    const DOM = {
      ...
      addTransaction(transaction, index) {
        ...
        image.addEventListener("click", () => {
          ...
          window.NotificateUser(`O Curso ${description} foi cancelado com sucesso, nos relate o motivo`);
          if (window.userAcceptedVoice) {
            window.notificateUserByVoice(
              `O Curso ${description} foi cancelado com sucesso, nos relate o motivo`
            );
          }
        });
    ...
    formElement.addEventListener("submit", (event) => {
      ...
      try {
        ...
        window.NotificateUser(
          `O Curso ${transaction.description} foi incluido na sua agenda`
        );
        if (window.userAcceptedVoice) {
          window.notificateUserByVoice(
            `O Curso ${transaction.description} foi incluido na sua agenda`
  ...
  function DarkMode() {
    ...
    function changeText(hasDarkMode,showNotification=true) {
      ...
      p.textContent = hasDarkMode ? "Cursos" : "Vagas ";
      darkModeCheckbox.checked = p.textContent === "Cursos";
      if(!showNotification) return;
      window.NotificateUser(
        `Perifa Acessa Buscar vaga que seja compatível com o meu perfil,  Você foi candidatado na Vaga com sucesso, a empresa entrará em contato pelos contatos informados}`
      );
      if (window.userAcceptedVoice) {
        window.notificateUserByVoice(
          `+Perifa Acessa Buscar vaga que seja compatível com o meu perfil,  Você foi candidatado na Vaga com sucesso, a empresa entrará em contato pelos contatos informados}`
  ...
  function DownloadJSON() {
    ...
    saveJsonButton.addEventListener("click", () => {
      ...
      downloadModal.classList.remove("active");
      window.NotificateUser(`Perifa Acessa Buscar Vaga por Estado${filename} de Desenvolvedor Mobile Pleno`);
      if (window.userAcceptedVoice) {
        window.notificateUserByVoice(`Perifa Acessa Buscar vaga que seja compatível com o meu perfil`);
      }
    });
  }
